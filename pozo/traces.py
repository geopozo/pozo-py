    ...
    def get_interval(self):
        ...
        for i in range(len(self.get_depth()) - 1):
            if self.get_depth()[i] == self.get_depth()[-1]:
                break
            start, stop, step = isClose(self.get_depth(), i)
            ...
            interval["sample_rate_consistent"] = sample_rate_consistent if sample_rate_consistent else None
            intervals.append(interval)
            if sample_rate_consistent is False: break
        return intervals
